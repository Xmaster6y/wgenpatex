...
def optim_patching(args):
    ...
    null_nb = 0
    target_img = imread(target_img_name)
    is_patching = torch.zeros(target_img.shape[2:], dtype=torch.bool, device=DEVICE)
    for i in range(target_img.shape[2]):
        for j in range(target_img.shape[3]):
            if torch.all(target_img[0,:,i,j] == torch.zeros(3, dtype=torch.float)):
                null_nb += 1
